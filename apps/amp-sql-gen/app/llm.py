...
def normalize_sql(sql: str) -> str:
    COLUMNS = (
        "up.id, up.member_id, up.sponsor_id AS parentId, "
        "up.status, up.registered_at, up.rank, up.tsv, mp.depth"
    )
    # Replace SELECT COUNT(*) or SELECT COUNT(whatever) with our column list
    pattern = re.compile(
        r"(?i)select\s+count\s*\(\s*[^)]*\s*\)\s*(?:as\s+\w+\s*)?",
        re.IGNORECASE,
    )
    if pattern.search(sql):
        sql = pattern.sub(f"SELECT {COLUMNS} ", sql)
    return sql

def enforce_limit(sql: str) -> str:
    # Look for an existing LIMIT clause
    pattern = re.compile(r'(?i)\blimit\s+(\d+)')
    match = pattern.search(sql)
    if match:
        user_limit = int(match.group(1))
        # Cap to MAX_LIMIT if user asks for more
        if user_limit > settings.LIMIT_MAX_CAP:
            return pattern.sub(f"LIMIT {settings.LIMIT_MAX_CAP}", sql, count=1)
        return sql
    # No LIMIT given -> append the default
    return f"{sql.rstrip(';')} LIMIT {settings.LIMIT_MAX};"